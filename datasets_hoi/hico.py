...
import datasets_hoi.transforms as T
...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, args):
        ...
        self.qwen3_embed_dir = Path('/host/space0/chen-j/gitClone/Hybrid-SOV-Qwen3-VL-Embed/data/hico_qwen3_embed/Qwen3-VL-Embedding-2B')
        config_file = str(getattr(args, "config_file", "") or "").lower()
        arch = str(getattr(args, "arch", "") or "").lower()
        self.use_qwen3_embed = "qwen" in config_file or "qwen" in arch
        self.img_rare_folder = Path('/home/yanai-lab/wang-y/space0/jupyter/HOI_task/SOV-STG-beta/data/HICO_rare2015/images')
        ...
        self.use_clip = True if "CLIP" in args.arch or "BLIP" in args.arch else False
        if self.use_clip:
    ...
    def __getitem__(self, idx):
        ...
        if 'rare' in img_anno['file_name']:
            img = Image.open(self.img_rare_folder / img_anno['file_name']).convert('RGB')
        else:
        ...
        if self.use_qwen3_embed:
            target['qwen3_embed'] = self._load_qwen3_embed(img_anno['file_name'])
    ...
    def _load_qwen3_embed(self, file_name):
        npy_name = Path(file_name).with_suffix(".npy")
        split_path = self.qwen3_embed_dir / self.img_set / npy_name
        flat_path = self.qwen3_embed_dir / npy_name

        if split_path.exists():
            embed_path = split_path
        elif flat_path.exists():
            embed_path = flat_path
        else:
            raise FileNotFoundError(
                f"Qwen3 embed not found for {file_name}. "
                f"Tried: {split_path} and {flat_path}"
            )
