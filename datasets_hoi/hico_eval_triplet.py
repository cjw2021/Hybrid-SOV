...
from datasets_hoi.hico_text_label import hico_text_label
...
class HICOEvaluator():
    def __init__(self, preds, gts, dataset_path, out_dir, epoch, bins_num=10, use_nms=True, nms_thresh=0.5, nms_alpha=1, nms_beta=0.5):
        ...
        for img_preds, img_gts in tqdm(zip(preds, gts), disable= not utils.is_main_process()):
            ...
            hoi_scores = img_preds['hoi_scores']
            ...
            if len(subject_ids) > 0:
                hois = [{'subject_id': subject_id, 'object_id': object_id, 'category_id': category_id,
                            'score': score}
                        for
                        subject_id, object_id, category_id, score in
                        zip(subject_ids[topk_indexes], object_ids[topk_indexes], hoi_labels[topk_indexes],
                            topk_hoi_scores)]
            else:
                hois = []
        ...
        for preds_i in tqdm(self.preds_t):
            ...
            for hoi in hois:
                ...
                hoi_id = hoi['category_id'] + 1
                hoi_id = "{:03d}".format(hoi_id)
