...
def resize2(image, target, size):
    rescaled_image = F.resize(image, [size, size])
    ...
    if "masks" in target:
        target['masks'] = interpolate(
            target['masks'][:, None].float(), [size, size], mode="nearest")[:, 0] > 0.5
...
class RandomResize2(object):
    ...
    def __call__(self, img, target=None):
        size = random.choice(self.sizes)
        return resize2(img, target, size)
