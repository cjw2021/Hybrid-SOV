# ------------------------------------------------------------------------
# SOV-STG-VLA
# Copyright (c) 2025 Junwen Chen. All Rights Reserved.
...
from datasets_hoi.vcoco_text_label import *
import json
...
class PostProcessHOI2(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        obj_prob = out_obj_logits.softmax(-1)  # bs, nq, cls
        ...
        # top 100
        sub_boxes = torch.gather(sub_boxes, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4))
        obj_boxes = torch.gather(obj_boxes, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4))
