...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0):
        ...
        self.cost_verb_class = weight_dict['cost_verb_class']
        self.cost_hoi_class = weight_dict['cost_hoi_class']
    ...
    def forward(self, outputs, targets):
        ...
        out_bbox = outputs["pred_obj_boxes"].flatten(0, 1)  # [batch_size * num_queries, 4]
        out_bbox_sub = outputs["pred_sub_boxes"].flatten(0, 1)  # [batch_size * num_queries, 4]

        # Also concat the target labels and boxes
        tgt_ids = torch.cat([v["obj_labels"] for v in targets])
        tgt_bbox = torch.cat([v["obj_boxes"] for v in targets])
        tgt_bbox_sub = torch.cat([v["sub_boxes"] for v in targets])
        ...
        cost_bbox_obj = torch.cdist(out_bbox, tgt_bbox, p=1)
        cost_bbox_sub = torch.cdist(out_bbox_sub, tgt_bbox_sub, p=1)

        cost_bbox = torch.stack((cost_bbox_sub, cost_bbox_obj)).max(dim=0)[0]

        # Compute the giou cost betwen boxes
        cost_giou_obj = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox), box_cxcywh_to_xyxy(tgt_bbox))
        cost_giou_sub = -generalized_box_iou(box_cxcywh_to_xyxy(out_bbox_sub), box_cxcywh_to_xyxy(tgt_bbox_sub))

        cost_giou = torch.stack((cost_giou_sub, cost_giou_obj)).max(dim=0)[0]
