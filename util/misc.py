...
def collate_fn2(batch):
    # import ipdb; ipdb.set_trace()
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list2(batch[0])
    return tuple(batch)
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor], is_vit=False):
    ...
    if tensor_list[0].ndim == 3:
        ...
        if is_vit:
            tensor = torch.zeros((b, c, 1024, 1024), dtype=dtype, device=device)
            mask = torch.ones((b, 1024, 1024), dtype=torch.bool, device=device)
        else:
...
def nested_tensor_from_tensor_list2(tensor_list: List[Tensor], is_vit=False):
    # TODO make this more general
    if tensor_list[0].ndim == 3:
        if torchvision._is_tracing():
            # nested_tensor_from_tensor_list() does not export well to ONNX
        ...
        tensor = torch.zeros((b, c, 640, 640), dtype=dtype, device=device)
        mask = torch.ones((b, 640, 640), dtype=torch.bool, device=device)
...
def init_distributed_mode(args):
    if 'WORLD_SIZE' in os.environ and os.environ['WORLD_SIZE'] != '': # 'RANK' in os.environ and 
        ...
        if args.rank == 0 and os.environ.get("SOV_PRINT_ENV") == "1":
