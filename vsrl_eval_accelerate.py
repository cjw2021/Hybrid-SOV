# AUTORIGHTS
# ------------------------------------------------------------------------
# Modified from SOV-STG-VLA (https://github.com/cjw2021/SOV-STG-VLA)
...
def _process_image_agent(image_data, actions, num_actions, ovr_thresh):
  """Process one image for agent evaluation - used in parallel processing"""
  gt_boxes = image_data['gt_boxes']
  gt_actions = image_data['gt_actions']
  pred_agents = image_data['pred_agents']
  
  tp = [[] for _ in range(num_actions)]
  fp = [[] for _ in range(num_actions)]
  sc = [[] for _ in range(num_actions)]
  ...
  return {'tp': tp, 'fp': fp, 'sc': sc}


def _process_image_role(image_data, actions, roles, num_actions, ovr_thresh, eval_type):
  """Process one image for role evaluation - used in parallel processing"""
  gt_boxes = image_data['gt_boxes']
  gt_actions = image_data['gt_actions']
  gt_role_id = image_data['gt_role_id']
  all_boxes = image_data['all_boxes']
  pred_agents = image_data['pred_agents']
  pred_roles = image_data['pred_roles']
  
  tp = [[[] for r in range(2)] for a in range(num_actions)]
  fp = [[[] for r in range(2)] for a in range(num_actions)]
  sc = [[[] for r in range(2)] for a in range(num_actions)]
  ...
  for aid in range(num_actions):
    ...
    for rid in range(len(roles[aid]) - 1):
      ...
      gt_role_inds = gt_role_id[:, aid, rid]
      gt_roles = -np.ones_like(gt_boxes)
      for j in range(gt_boxes.shape[0]):
        if gt_role_inds[j] > -1:
          gt_roles[j] = all_boxes[gt_role_inds[j]]
...
class VCOCOeval(object):
  ...
  def _collect_detections_for_image(self, image_id):
    """Optimized version using prebuilt dictionary"""
    if image_id not in self.dets_dict:
      agents = np.empty((0, 4 + self.num_actions), dtype=np.float32)
      roles = np.empty((0, 5 * self.num_actions, 2), dtype=np.float32)
      return agents, roles
    
    dets = self.dets_dict[image_id]
    num_dets = len(dets)
    
    agents = np.zeros((num_dets, 4 + self.num_actions), dtype=np.float32)
    roles = np.zeros((num_dets, 5 * self.num_actions, 2), dtype=np.float32)
    
    for i, det in enumerate(dets):
      agents[i, :4] = det['person_box']
      for aid in range(self.num_actions):
        for j, rid in enumerate(self.roles[aid]):
          if rid == 'agent':
            agents[i, 4 + aid] = det[self.actions[aid] + '_' + rid]
          else:
            roles[i, 5 * aid: 5 * aid + 5, j-1] = det[self.actions[aid] + '_' + rid]
  ...
  def _do_eval(self, detections_file, ovr_thresh=0.5):
    ...
    # Create detection lookup dictionary for faster access
    print("Building detection lookup dictionary...")
    self.dets_dict = {}
    for det in self.dets:
      img_id = det['image_id']
      if img_id not in self.dets_dict:
        self.dets_dict[img_id] = []
      self.dets_dict[img_id].append(det)
    
    self._do_agent_eval(vcocodb, ovr_thresh=ovr_thresh)
  ...
  def _do_role_eval(self, vcocodb, ovr_thresh=0.5, eval_type='scenario_1'):
    """Optimized role evaluation with parallel processing"""
    ...
    print(f"Processing {len(vcocodb)} images for role evaluation [{eval_type}]...")
    
    process_func = partial(_process_image_role,
                          actions=self.actions,
                          roles=self.roles,
                          num_actions=self.num_actions,
                          ovr_thresh=ovr_thresh,
                          eval_type=eval_type)
    ...
    for i in range(len(vcocodb)):
      ...
      gt_actions = vcocodb[i]['gt_actions'][gt_inds]
      gt_role_id = vcocodb[i]['gt_role_id'][gt_inds]
      all_boxes = vcocodb[i]['boxes']
      
      pred_agents, pred_roles = self._collect_detections_for_image(image_id)
      
      image_data.append({
        'gt_boxes': gt_boxes,
        'gt_actions': gt_actions,
        'gt_role_id': gt_role_id,
        'all_boxes': all_boxes,
    ...
    if self.num_workers > 1:
      ...
      results = [process_func(data) for data in image_data]
    
    # Aggregate results
    for result in results:
      for aid in range(self.num_actions):
        for rid in range(2):
          tp[aid][rid].extend(result['tp'][aid][rid])
          fp[aid][rid].extend(result['fp'][aid][rid])
          sc[aid][rid].extend(result['sc'][aid][rid])
  ...
  def _do_agent_eval(self, vcocodb, ovr_thresh=0.5):
    """Optimized agent evaluation with parallel processing"""
    ...
    # Parallel processing for images
    print(f"Processing {len(vcocodb)} images with {self.num_workers} workers...")
    
    process_func = partial(_process_image_agent, 
                          actions=self.actions,
                          num_actions=self.num_actions,
                          ovr_thresh=ovr_thresh)
    ...
    for i in range(len(vcocodb)):
      ...
      gt_boxes = vcocodb[i]['boxes'][gt_inds]
      gt_actions = vcocodb[i]['gt_actions'][gt_inds]
      pred_agents, _ = self._collect_detections_for_image(image_id)
    ...
    for result in results:
      for aid in range(self.num_actions):
        tp[aid].extend(result['tp'][aid])
        fp[aid].extend(result['fp'][aid])
        sc[aid].extend(result['sc'][aid])
...
def get_overlap(boxes, ref_box):
  """Vectorized IoU computation - optimized version"""
...
def batch_get_overlap(boxes1, boxes2):
  """Compute IoU between two sets of boxes in a vectorized manner
  boxes1: (N, 4)
  boxes2: (M, 4)
  ...
  ixmin = np.maximum(boxes1_exp[:, :, 0], boxes2_exp[:, :, 0])
  iymin = np.maximum(boxes1_exp[:, :, 1], boxes2_exp[:, :, 1])
  ixmax = np.minimum(boxes1_exp[:, :, 2], boxes2_exp[:, :, 2])
  iymax = np.minimum(boxes1_exp[:, :, 3], boxes2_exp[:, :, 3])
  ...
  # Union
  uni = area1[:, None] + area2[None, :] - inters
...
if __name__ == "__main__":
  ...
  det_file_path = os.path.join(args.detections, 'vcoco.pickle')
  vcocoeval = VCOCOeval(vsrl_annot_file, coco_annot_file, split_file)
  vcocoeval._do_eval(det_file_path, ovr_thresh=0.5)